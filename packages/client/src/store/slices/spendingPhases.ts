...
export interface SpendingPhase {
  ...
  startYear: number;
  endYear: number;
  minMonthlySpend: number; // integer cents
  maxMonthlySpend: number; // integer cents
}
...
export const createSpendingPhasesSlice: StateCreator<SpendingPhasesSlice> = (
  set
) => ({
  spendingPhases: [
    {
  ...
  addSpendingPhase: () =>
    set((state) => {
      if (state.spendingPhases.length >= 4) return state;
      ...
      if (duration <= 1) return state; // Cannot split 1-year phase
  ...
  updateSpendingPhase: (id, updates) =>
    set((state) => {
      ...
      for (let i = 0; i < phases.length; i++) {
        if (i === 0) phases[i].startYear = 1;
        else phases[i].startYear = phases[i - 1].endYear + 1;
  ...
  removeSpendingPhase: (id) =>
    set((state) => {
      if (state.spendingPhases.length <= 1) return state;
