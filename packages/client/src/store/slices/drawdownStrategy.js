...
export const createDrawdownStrategySlice = (set) => ({
    drawdownStrategy: {
        type: 'bucket',
    ...
    setDrawdownStrategyType: (type) => set((state) => ({ drawdownStrategy: { ...state.drawdownStrategy, type } })),
    setBucketOrder: (bucketOrder) => set((state) => ({ drawdownStrategy: { ...state.drawdownStrategy, bucketOrder } })),
    updateRebalancingAllocation: (targetAllocation) => set((state) => ({
        drawdownStrategy: {
            ...
            rebalancing: {
                ...
                targetAllocation,
                // Sync first waypoint
                glidePath: state.drawdownStrategy.rebalancing.glidePath.map((wp, i) => i === 0 ? { ...wp, allocation: targetAllocation } : wp)
            },
        },
    })),
    setGlidePathEnabled: (glidePathEnabled) => set((state) => ({
        drawdownStrategy: {
            ...state.drawdownStrategy,
            rebalancing: { ...state.drawdownStrategy.rebalancing, glidePathEnabled },
        },
    })),
    addGlidePathWaypoint: () => set((state) => {
        const { glidePath } = state.drawdownStrategy.rebalancing;
        if (glidePath.length >= 10)
            return state;
    ...
    updateGlidePathWaypoint: (index, updates) => set((state) => {
    ...
    removeGlidePathWaypoint: (index) => set((state) => {
        const { glidePath } = state.drawdownStrategy.rebalancing;
