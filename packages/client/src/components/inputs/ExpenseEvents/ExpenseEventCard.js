...
export const ExpenseEventCard = ({ event, onUpdate, onRemove, }) => {
    ...
    return (_jsxs("div", { className: "relative flex flex-col gap-4 p-3 bg-white border border-slate-200 rounded-lg shadow-sm border-l-4 border-l-rose-500", children: [_jsxs("div", { className: "flex items-center justify-between gap-4", children: [_jsxs("div", { className: "flex items-center gap-2 flex-1", children: [_jsx(ArrowUpCircle, { size: 16, className: "text-rose-500 shrink-0" }), _jsx("input", { type: "text", value: event.name, onChange: (e) => onUpdate(event.id, { name: e.target.value }), placeholder: "Expense name...", className: "bg-transparent text-[13px] font-semibold text-slate-700 outline-none hover:bg-slate-50 px-1 rounded transition-colors w-full" })] }), _jsx("button", { onClick: () => onRemove(event.id), className: "text-slate-400 hover:text-red-500 transition-colors", children: _jsx(X, { size: 14 }) })] }), _jsxs("div", { className: "grid grid-cols-2 gap-3", children: [_jsx(NumericInput, { label: "Amount", value: event.amount / 100, onChange: (val) => onUpdate(event.id, { amount: val * 100 }), format: "currency", prefix: "$" }), _jsx(NumericInput, { label: "Duration (Months)", value: event.durationMonths, onChange: (val) => onUpdate(event.id, { durationMonths: val }), min: 1, max: 480 })] }), _jsxs("div", { className: "grid grid-cols-2 gap-3", children: [_jsx(MonthYearPicker, { label: "Start Date", month: startM, year: startY, onChange: handleDateChange }), _jsx("div", { className: "flex items-end pb-1", children: _jsx(ToggleSwitch, { label: "CPI Linked", enabled: event.escalation.kind === 'cpiLinked', onChange: (val) => onUpdate(event.id, { escalation: { kind: val ? 'cpiLinked' : 'none' } }) }) })] })] }));
};
