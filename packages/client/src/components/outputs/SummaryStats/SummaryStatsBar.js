...
export const SummaryStatsBar = () => {
    ...
    if (!activeResult || status === 'idle' || status === 'running') {
        return (_jsx("div", { className: `grid grid-cols-2 md:grid-cols-4 ${simulationMode === SimulationMode.MONTE_CARLO ? 'xl:grid-cols-9' : 'xl:grid-cols-8'} gap-3 w-full animate-pulse`, children: [...Array(cardCount)].map((_, i) => (_jsx("div", { className: "h-20 bg-white border border-slate-100 rounded-xl" }, i))) }));
    }
