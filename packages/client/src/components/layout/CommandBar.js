...
export const CommandBar = () => {
    ...
    const handleRun = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const details = errorData.error?.details ? `: ${errorData.error.details.map((d) => `${d.path} ${d.message}`).join(', ')}` : '';
    ...
    const getButtonContent = () => {
        if (simulationResults.status === 'running')
            return 'Running...';
        if (simulationResults.status === 'error')
            return 'Error - Try Again';
        return 'Run Simulation';
    };
    return (_jsxs("header", { className: "h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0", children: [_jsxs("div", { className: "flex items-center gap-8", children: [_jsxs("div", { className: "flex flex-col gap-0.5", children: [_jsx("span", { className: "text-[10px] font-bold text-slate-400 uppercase tracking-widest", children: "View Mode" }), _jsx(SegmentedToggle, { options: [
          ...
          `, children: [_jsx(Play, { size: 16, fill: "currentColor" }), getButtonContent()] }) })] }));
};
